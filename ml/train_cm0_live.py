#!/usr/bin/env python3
"""
train_cm0_live.py: CM0 (Complex-Valued VAE, Raw Labels)
...
try:
    from models.cm0 import CM0_ComplexAutoEncoder
    ...
    from ml.models.cm0 import CM0_ComplexAutoEncoder
...
def train_epoch(model, optimizer, bridge, batch_size, device, epoch_num, num_batches, cfg, global_step_offset):
    ...
    CM0 uses labels: 0, 1, 2 (easy_composite, prime, hard_composite)
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="CM0 Complex VAE - Live Training (Complex-Valued Latent Space)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python3 -m ml.train_cm0_live --epochs 5 --batches-per-epoch 100 --batch-size 32
  python3 -m ml.train_cm0_live --epochs 10 --batches-per-epoch 5000 --batch-size 64 --lr 0.0005
        """
    )
    ...
    parser.add_argument("--checkpoint-dir", default="checkpoints_cm0", help="Checkpoint directory")
    ...
    logger.info("=" * 80)
    logger.info("🚀 CM0 Complex VAE - Live Training (Complex-Valued Latent Space)")
    logger.info("="*80)
    ...
    model = CM0_ComplexAutoEncoder(cfg)
    ...
    logger.info("="*80)
    logger.info(f"Mode: CM0 (Labels: 0=easy_composite, 1=prime, 2=hard_composite)")
