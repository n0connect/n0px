#!/usr/bin/env python3
"""
train_rm1_live.py: RM1 (Real-Valued VAE, Differential Privacy Labels)
...
try:
    from models.rm1 import RM1_RealAutoEncoder
    ...
    from ml.models.rm1 import RM1_RealAutoEncoder
...
def train_epoch(model, optimizer, bridge, batch_size, device, epoch_num, num_batches, cfg, global_step_offset):
    ...
    RM1 uses labels: 3, 4, 5 (dp_prime, dp_composite, dp_hard_composite)
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="RealNoiseMixtureVAE - RM1 Live Training",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python3 -m ml.train_rm1_live --epochs 5 --batches-per-epoch 100 --batch-size 32
  python3 -m ml.train_rm1_live --epochs 10 --batches-per-epoch 5000 --batch-size 64 --lr 0.0005
        """
    )
    ...
    parser.add_argument("--checkpoint-dir", default="checkpoints_rm1", help="Checkpoint directory")
    ...
    logger.info("=" * 80)
    logger.info("🚀 RealNoiseMixtureVAE - RM1 (DP Labels) LIVE TRAINING")
    logger.info("="*80)
    ...
    logger.info("="*80)
    logger.info(f"Mode: RM1 (Labels: 3=dp_prime, 4=dp_composite, 5=dp_hard_composite)")
    ...
    # === SEPARABILITY LOSS STATUS ===
    sep_cfg = cfg.get("sep_loss", {})
    if sep_cfg and sep_cfg.get("enabled", False):
        logger.info(f"\n✓ Separability Loss ENABLED:")
        logger.info(f"  - Lambda (base): {sep_cfg.get('lambda', 0.05)}")
        logger.info(f"  - Warmup steps: {sep_cfg.get('warmup_steps', 500)}")
        logger.info(f"  - Ramp steps: {sep_cfg.get('ramp_steps', 1500)}")
        logger.info(f"  - Min per class: {sep_cfg.get('min_per_class', 8)}")
    else:
        logger.warning("⚠ Separability Loss DISABLED (sep_loss.enabled=false)")
